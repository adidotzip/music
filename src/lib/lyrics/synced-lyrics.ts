...
const CACHE_VERSION = 12 // Bumped to preserve translations, romanization, and instrumental flags
...
export const mapBraccatoToSyncedLyrics = (lyrics: Lyric[]): SyncedLyricsLine[] => {
    return lyrics.map((lyric) => {
        ...
        if (lyric.translations && Object.keys(lyric.translations).length > 0) {
            ...
            // Deterministic translation selection
            const preferred = 
                translations.en ?? 
...
const parseBraccatoLyrics = (
    ...
    if (parsed && parsed.length > 0) {
        ...
        const hasWordTiming = parsed.some((lyric) => lyric.parts?.length && lyric.parts.length > 1)
        return {
            status: 'found',
            source,
            lines,
...
const getLrclibFoundResult = (
    ...
    return parseBraccatoLyrics(data.syncedLyrics, durationSeconds * 1000, 'lrclib')
}
...
const scoreAdiSearchResult = (data: any, track: TrackData): number => {
    const expectedTrackName = normalizeSearchText(track.name)
    const expectedArtistName = normalizeSearchText(formatArtists(track.artists))
    const resultTrackName = normalizeSearchText(data.title ?? data.name ?? '')
    const resultArtistName = normalizeSearchText(data.artist ?? data.artistName ?? '')
...
const fetchAdiLyrics = async (
    ...
    try {
        ...
        const bestMatch = searchData.results
            .map((item: any) => ({ item, score: scoreAdiSearchResult(item, track) }))
        ...
        return parseBraccatoLyrics(lyricData.lyric.rawContent, durationMs, 'adi')
    } catch (error) {
        if (error instanceof Error && error.name === 'AbortError') throw error
        console.warn('[Lyrics] fetchAdiLyrics failed:', error)
        return { status: 'error' }
    }
}
...
const fetchBetterLyrics = async (
    ...
    try {
        ...
        const durationMs = getDurationSeconds(track) * 1000
        return parseBraccatoLyrics(data.ttml, durationMs, 'better')
    } catch (error) {
        if (error instanceof Error && error.name === 'AbortError') throw error
        console.warn('[Lyrics] fetchBetterLyrics failed:', error)
        return { status: 'error' }
    }
}
...
const normalizeLine = (value: unknown): SyncedLyricsLine | undefined => {
    ...
    return { 
        ...
        translations: isRecord(value.translations)
            ? Object.fromEntries(
            ...
            : undefined,
        romanization: typeof value.romanization === 'string' ? value.romanization : undefined,
        isInstrumental: typeof value.isInstrumental === 'boolean' ? value.isInstrumental : undefined,
    }
}
    ...
    if (Array.isArray(data.lyrics)) {
        ...
        const lines = data.lyrics
            .map((line): SyncedLyricsLine | undefined => {
                ...
                const syllabusArray = Array.isArray(line.syllabus) ? line.syllabus : (Array.isArray(line.syllables) ? line.syllables : null)
                
                if (syllabusArray) {
                    const words = syllabusArray
                        .map((syl) => {
                            if (
                                !isRecord(syl) ||
...
const fetchLyricsPlusLyrics = async (
    ...
    for (const source of sources) {
        try {
            ...
            if (error instanceof Error && error.name === 'AbortError') throw error
            console.warn(`[Lyrics] fetchLyricsPlusLyrics (${source}) failed:`, error)
        }
    }
...
const fetchLrclibLyrics = async (
    ...
    try {
        ...
        if (error instanceof Error && error.name === 'AbortError') throw error
        console.warn('[Lyrics] fetchLrclibLyrics failed:', error)
        return { status: 'error' }
    }
}
...
const fetchUnisonLyrics = async (
    ...
    try {
        ...
        const durationMs = getDurationSeconds(track) * 1000
        return parseBraccatoLyrics(payload.data.lyrics, durationMs, 'unison')
    } catch (error) {
        if (error instanceof Error && error.name === 'AbortError') throw error
        console.warn('[Lyrics] fetchUnisonLyrics failed:', error)
        return { status: 'error' }
    }
}
