...
const fetchAdiLyrics = async (
    ...
    try {
        ...
        if (!lyricData.ok || !lyricData.lyric || !lyricData.lyric.eslrc) {
            return { status: 'not-found' }
        }
...
export const fetchSyncedLyrics = async (
    ...
    try {
        ...
        if (lpResult && lpResult.lines.length > 0) {
            const finalResult: SyncedLyricsResult = {
                status: 'found',
                source: 'lyricsplus',
                ...lpResult,
            }
            await saveLyricsToCache(track.id, finalResult)
            return finalResult
        }

        const unisonResult = await fetchUnisonLyrics(track, signal)
        ...
        const notFound: SyncedLyricsResult = { status: 'not-found' }
        await saveLyricsToCache(track.id, notFound)
        return notFound
    } catch (error) {
        if (error instanceof Error && error.name === 'AbortError') throw error
        return cachedResult || { status: 'not-found' }
    }
}
