...
import { type CachedLyricsResult, LyricsCache } from './LyricsCache.ts'
...
export function getSourceDisplayName(source?: string): string {
	...
	if (s === 'lyrics-plus' || s === 'lyricsplus') {
		return 'Lyrics+'
	}
	if (s === 'musixmatch') {
		return 'Musixmatch'
	}
	if (s === 'apple' || s === 'apple-music') {
		return 'Apple Music'
	}
	if (s === 'unison') {
		return 'Unison'
			...
			try {
				...
				if (lrcmuxResponse) {
					if (!lrcmuxResponse.isPlainOnly) {
						...
						const hasWordTiming = lyrics.some(
							(lyric) => lyric.parts && lyric.parts.length > 0,
						)
						const result: ServiceLyricsResult = {
							status: 'found',
							source: 'lrcmux',
